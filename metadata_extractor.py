...
def call_claude_api(messages, max_tokens=1000):
    ...
    try:
        ...
        if not api_key:
            print_status("No API key found. Falling back to manual metadata entry.", "warning")
            return None
...
def generate_title_from_purpose(purpose_text):
    ...
    if 'high agency' in purpose.lower():
        ...
        if len(words) >= 3:
            ...
            key_words = [w for w in words[:5] if w.lower() not in ['to', 'the', 'a', 'an', 'and', 'or', 'but']]
            if key_words:
...
def extract_technical_notes(content):
    ...
    content_lower = content.lower()
    if 'technical implementation' in content_lower or 'implementation notes' in content_lower:
        # Extract the section
        ...
        if start_idx != -1:
            ...
            section_content = content[start_idx:start_idx+1000]  # Take reasonable chunk
    ...
    if not any([technical_data['recommended_llm'], technical_data['temperature'], technical_data['max_tokens']]):
        # Analyze content to suggest appropriate defaults
        if len(content) > 2000 or 'complex' in content.lower() or 'advanced' in content.lower():
            technical_data['additional_notes'].append('Complex prompt - consider Claude 3.5 Sonnet with temperature 0.3-0.7')
        elif 'creative' in content.lower() or 'inspire' in content.lower() or 'generate' in content.lower():
            technical_data['additional_notes'].append('Creative prompt - consider higher temperature (0.7-0.9)')
        elif 'analysis' in content.lower() or 'precise' in content.lower() or 'accurate' in content.lower():
            technical_data['additional_notes'].append('Analytical prompt - consider lower temperature (0.1-0.4)')
